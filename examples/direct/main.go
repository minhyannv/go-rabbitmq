...
func main() {
	...
	ex := fmt.Sprintf("ex_direct_%d", suffix)
	q1 := fmt.Sprintf("q_direct_k1_%d", suffix)
	q2 := fmt.Sprintf("q_direct_k2_%d", suffix)
	...
	if err := c.BindQueue(ctx, q1, "k1", ex, rabbitmq.BindOptions{}); err != nil {
		log.Fatalf("bind q1: %v", err)
	}
	if err := c.BindQueue(ctx, q2, "k2", ex, rabbitmq.BindOptions{}); err != nil {
	...
	log.Printf("publishing to direct exchange %q routingKey=%q", ex, "k1")
	if err := c.PublishString(ctx, ex, "k1", "hello-direct-k1"); err != nil {
	...
	select {
	case m := <-got2:
		log.Fatalf("unexpected: q2 got: %s", m)
	case <-time.After(800 * time.Millisecond):
		log.Printf("[q2] got nothing (expected)")
	}
}
